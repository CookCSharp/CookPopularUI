...
namespace CookPopularUI.WPF.Controls
{
    public class ArcGeometrySource : GeometrySource<IArcGeometrySourceParameters>
    {
        ...
        protected override Rect ComputeLogicalBounds(Rect layoutBounds, IGeometrySourceParameters parameters)
        {
            ...
            return logicalBound.GetStretchBound(parameters.Stretch, new Size(1.0, 1.0));
        }
        ...
        private bool UpdateZeroAngleGeometry(bool relativeMode, double angle)
        {
            ...
            {
                double intersect = InnerCurveSelfIntersect(num, num2, this.absoluteThickness);
            ...
            flag |= GeometryHelper.EnsureGeometryType<LineGeometry>(out LineGeometry? dependencyObject, ref this.cachedGeometry, () => new LineGeometry());
        ...
        private bool UpdatePieGeometry(double start, double end)
        {
            ...
            if (pathGeometry == null || pathGeometry.Figures.Count != 1 || (pathFigure = pathGeometry.Figures[0]).Segments.Count != 2 || (arcSegment = (ArcSegment)pathFigure.Segments[0]) == null || (dependencyObject = (LineSegment)pathFigure.Segments[1]) == null)
            {
        ...
        private bool UpdateOpenArcGeometry(double start, double end)
        {
            ...
            PathFigure pathFigure;
            ArcSegment arcSegment;
            if (pathGeometry == null || pathGeometry.Figures.Count != 1 || (pathFigure = pathGeometry.Figures[0]).Segments.Count != 1 || (arcSegment = (ArcSegment)pathFigure.Segments[0]) == null)
            {
                ((PathGeometry)(this.cachedGeometry = new PathGeometry())).Figures.Add(pathFigure = new PathFigure());
                pathFigure.Segments.Add(arcSegment = new ArcSegment());
        ...
        private static IList<Point> ComputeOneInnerCurve(double start, double end, Rect bounds, double offset)
        {
            ...
            for (int j = 1; j < num4; j++)
            {
                point2 = list[j - 1];
                point3 = list[j];
                double scalar = point2.Distance(point3) / 3.0;
