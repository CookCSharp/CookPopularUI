/*
 *Description: WindowExtensions
 *Author: Chance.zheng
 *Creat Time: 2023/8/23 16:20:54
 *.Net Version: 4.6
...
namespace CookPopularToolkit.Windows
{
    ...
    public static class WindowExtensions
    {
        ...
        public static bool HwndCreatedButNotShown(this Window window) => CreateGetFieldValueDelegate<Window, bool>("_hwndCreatedButNotShown").Invoke(window);
        ...
        /// <param name="window">一个 <see cref="Window" /> 对象。</param>
        /// <returns>若是，返回<c>true</c>；否则，返回<c>false</c>。</returns>
        public static bool IsClosing(this Window window) => CreateGetFieldValueDelegate<Window, bool>("_isClosing").Invoke(window);
        ...
        /// <param name="window">一个 <see cref="Window" /> 对象。</param>
        /// <returns>若是，返回<c>true</c>；否则，返回<c>false</c>。</returns>
        public static bool IsDisposed(this Window window) => CreateGetFieldValueDelegate<Window, bool>("_disposed").Invoke(window);
        ...
        /// <param name="window">一个 <see cref="Window" /> 对象。</param>
        /// <returns>若是，返回<c>true</c>；否则，返回<c>false</c>。</returns>
        public static bool ShowingAsDialog(this Window window) => CreateGetFieldValueDelegate<Window, bool>("_showingAsDialog").Invoke(window);
        ...
        public static void RemoveIcon(this Window window)
        {
            ...
            InteropMethods.SetWindowLong(hwnd, (int)InteropValues.GetWindowLongFields.GWL_EXSTYLE, new IntPtr(extendedStyle | InteropValues.WS_EX_DLGMODALFRAME));
        ...
        private static bool SetTopmostCore(IntPtr hWnd, bool topmost)
        {
            try
            {
                ...
                if (InteropMethods.SetWindowPos(hWnd, topmost ? new IntPtr(InteropValues.HWND_TOPMOST) : new IntPtr(InteropValues.HWND_NOTOPMOST), 0, 0, 0, 0, uFlags))
                {
                    var exStyleWithTopmost = InteropMethods.GetWindowLong(hWnd, (int)InteropValues.GetWindowLongFields.GWL_EXSTYLE) & InteropValues.WS_EX_TOPMOST;

                    return topmost ? (exStyleWithTopmost == InteropValues.WS_EX_TOPMOST) : (exStyleWithTopmost != InteropValues.WS_EX_TOPMOST);
                }
        ...
        public static bool SetTopmost(this Window window, bool topmost, int millisecondsTimeout = 0)
        {
            ...
            return topmost ? SpinWait.SpinUntil(new Func<bool>(() => SetTopmostCore(hWnd, false)), millisecondsTimeout)
                          && SpinWait.SpinUntil(new Func<bool>(() => SetTopmostCore(hWnd, true)), millisecondsTimeout)
        ...
        /// </summary>
        /// <param name="window"></param>
        public static void SetWindowToForeground(this Window window)
        {
